...
Your goal is to follow the `INSTRUCTIONS` section and specify actions to perform on the files to achieve objective in `[TASK]` section.
...
1. **Success Criteria**: Define how the success of achieving the `[TASK]` section can be verified with automated tests.
